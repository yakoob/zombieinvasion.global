...
class InboundChannelActor extends BaseActor implements ObjectBinding, InboundChannelState {
    ...
    public InboundChannelActor(String channelUuid, String destination) {
        ...
        state_call_ChannelState__fsm.record().on([Parked]).from(None).to(None).act = {

            log.info("on Parked from None -> None")
            state_call_ChannelState__fsm.goToState(state_call_ChannelState__fsm.currentValue)

        }

        // Initialize a Greeting
        state_call_ChannelState__fsm.record().on([Init]).from(Greeting).to(Greeting).act = { event ->
            // play greeting
            audioActorService.actorRef.tell(new Shout(channelUuid: channelUuid, audioId: state_call_ChannelState__fsm.currentValue.greeting.id), context.self())
        }

        // Play a menu Greeting
        state_call_ChannelState__fsm.record().on([Init]).from(Menu).to(Menu).act = { event ->
            // play menu greeting
            audioActorService.actorRef.tell(new Shout(channelUuid: channelUuid, audioId: state_call_ChannelState__fsm.currentValue.greeting.id), akkaService.actorNoSender())
        }

        // Wait for menu events or other state events
        state_call_ChannelState__fsm.record().on([KeyPressed]).from(Menu).to(Menu).act = { event ->
            speechActorService.actorRef.tell(new Speak(channelUuid: channelUuid, words: "dtmf ${event.digit}"), akkaService.actorNoSender())
        }

        state_call_ChannelState__fsm.record().on([Hungup]).from(All).to(All).act = { event ->
    ...
    void onReceive(Object message) throws Exception {
        ...
        state_call_ChannelState__fsm.fire(message, state_call_ChannelState__fsm.currentState)
